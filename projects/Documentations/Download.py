...
DOC_FOLDER = "data/documents/pdf_files"

def get_all_files(root_path):
    file_structure = {}
    allowed_extensions = (".pdf", ".png", ".jpg", ".jpeg")
    for root, dirs, files in os.walk(root_path):
        valid_files = [f for f in files if f.lower().endswith(allowed_extensions)]
        if valid_files:
...
def show():
    ...
    # Hiển thị kết quả (giữ nguyên cấu trúc của bạn)
        ...
        with st.expander(f"📁 **{root_cat.upper()}**", expanded=is_expanded):
            ...
            for path in sorted(sub_items.keys()):
                ...
                for file_name, file_path in files:
                    ...
                    icon = "🖼️" if file_name.lower().endswith(('.png', '.jpg', '.jpeg')) else "📄"
                    with col1:
